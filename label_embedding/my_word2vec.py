...
def train_sentence_label(model, sentence, alpha, label, work=None, neu1=None):
    """
    Update CBOW model by training on a single sentence.
    The sentence is a list of string tokens, which are looked up in the model's
    vocab dictionary. Called internally from `Word2Vec.train()`.
    ...
    for pos, word in enumerate(word_vocabs):
        ...
        if word2_indices and model.cbow_mean:
            l1 /= len(word2_indices)
        train_cbow_pair_by_label(model, word, word2_indices, l1, alpha, label)

    return len(word_vocabs)


STAR_LABELS = ["STAR_LABEL1", "STAR_LABEL2", "STAR_LABEL3", "STAR_LABEL4", "STAR_LABEL5"]


def train_cbow_pair_by_label(model, word, input_word_indices, l1, alpha, label, learn_vectors=True, learn_hidden=True):
    ...
    if model.negative:
        ...
        word_indices = [0, 1, 2, 3, 4, word.index]
        while len(word_indices) < model.negative + 1:
            w = model.cum_table.searchsorted(model.random.randint(model.cum_table[-1]))
            if w != word.index:
                word_indices.append(w)
        l2b = model.syn1label[word_indices]  # 2d matrix, k+1 x layer1_size
        fb = 1. / (1. + exp(-dot(l1, l2b.T)))  # propagate hidden -> output
        label_index = int(label[-1]) - 1
        model.neg_labels[label_index] = 1
        gb = (model.neg_labels - fb) * alpha  # vector of error gradients multiplied by the learning rate
        if learn_hidden:
            model.syn1label[word_indices] += outer(gb, l1)  # learn hidden -> output
...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(
            ...
            trim_rule=None, sorted_vocab=1, train_by_label=False):
        """
    ...
    def _do_train_job(self, job, alpha, inits):
        ...
        for sentence in job:
            if self.train_by_label:
                sentence = sentence[:-1]
                label = sentence[:-1]
                tally += train_cbow_pair_by_label(self, sentence, alpha, work, neu1, label=label)
            elif self.sg:
    ...
    def train(self, sentences, total_words=None, word_count=0, chunksize=100, total_examples=None, queue_factor=2, report_delay=1):
        ...
        if FAST_VERSION < 0:
            ...
            if self.negative > 0:
                ...
                if self.train_by_label:
                    self.neg_labels = zeros(self.negative + 6)
                    self.neg_labels[5] = 1.
                else:
    ...
    def reset_weights(self):
        ...
        if self.train_by_label:
            self.syn1label = zeros((len(self.vocab) + 5, self.layer1_size), dtype=REAL)
